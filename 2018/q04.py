...
def a(data):
  ...
  return int(slept_most.lstrip('#')) * Counter(minutes_asleep[slept_most]).most_common(1)[0][0]

def b(data):
  minutes_asleep, sleep_minutes = helper(data)
  most_slept_count = {}
  for key in sleep_minutes:
  ...
  return int(guard_no.lstrip('#')) * most_slept_count[most][0]
